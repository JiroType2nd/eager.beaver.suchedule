...
export default function AttendPage() {
  ...
  if (!data) return <main className="min-h-screen p-4 bg-navy-900"><p className="text-slate-400">リンクが見つかりません</p></main>;

  return (
    <main className="min-h-screen pb-24 bg-navy-900">
      <div className="max-w-lg mx-auto px-4 py-6">
        <h1 className="text-xl font-bold text-white mb-1">出欠確認</h1>
        <p className="text-sm text-slate-400 mb-4">{data.teamName}</p>
        ...
        {mode === 'view' ? (
          <>
            <div className="mb-6">
              <button
                onClick={openForm}
                className="w-full py-3 bg-gold-500 text-navy-900 rounded-lg font-medium hover:bg-gold-400"
              >
                出欠を登録する
              </button>
            </div>
            ...
            <section>
              <h2 className="text-sm font-semibold text-slate-400 uppercase mb-2">各日程の集計</h2>
              <div className="space-y-3 mb-6">
                {data.activities.map((act) => {
                  const agg = getAggregate(act.id);
                  return (
                    <div key={act.id} className="p-4 bg-navy-800/80 border border-navy-600 rounded-lg">
                      <p className="font-medium text-white">{act.title || act.placeName}</p>
                      <p className="text-sm text-slate-400">{formatDateTimeRange(act.startAt, act.endAt)}</p>
                      <p className="text-sm text-slate-500">{act.placeName}</p>
                      <p className="flex gap-3 mt-2 text-sm">
                        <span className="text-emerald-400">〇 {agg.yes}人</span>
                        <span className="text-amber-400">△ {agg.maybe}人</span>
                        <span className="text-slate-400">× {agg.no}人</span>
                      </p>
                    </div>
                  );
                })}
            ...
            <section>
              <h2 className="text-sm font-semibold text-slate-400 uppercase mb-2">出欠一覧</h2>
              {data.submissions.length === 0 ? (
                <p className="text-slate-500 py-4">まだ登録はありません</p>
              ) : (
                <div className="space-y-2">
                  {data.submissions.map((sub) => (
                    <button
                      ...
                      className="w-full p-4 text-left bg-navy-800/60 border border-navy-600 rounded-lg hover:bg-navy-800/80 transition"
                    >
                      <p className="font-medium text-white">{sub.nickname}</p>
                      ...
                      <p className="text-xs text-slate-500 mt-1">タップして編集</p>
                    </button>
                  ))}
                </div>
              )}
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-1">ニックネーム *</label>
              <input
                type="text"
                value={nickname}
            ...
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-2">各日程の出欠 *</label>
              <div className="space-y-3">
                {data.activities.map((act) => (
                  <div key={act.id} className="p-4 bg-navy-800/60 border border-navy-600 rounded-lg">
                    <p className="font-medium text-sm text-white">{act.title || act.placeName}</p>
                    <p className="text-xs text-slate-500">{formatDateTimeRange(act.startAt, act.endAt)} {act.placeName}</p>
                        ...
                        <button
                          ...
                          className={`flex-1 py-2 rounded-lg text-sm font-medium transition ${
                            answers[act.id] === a
                              ...
                              : 'bg-navy-700/80 text-slate-400 hover:bg-navy-700 border border-navy-600'
                          }`}
                        >
                          {ANSWER_LABELS[a]}
                        </button>
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 py-3 border border-navy-500 rounded-lg font-medium text-slate-300 hover:bg-navy-800"
              >
                キャンセル
              </button>
              <button
                ...
                disabled={submitting}
                className="flex-1 py-3 bg-gold-500 text-navy-900 rounded-lg font-medium disabled:opacity-50 hover:bg-gold-400"
              >
